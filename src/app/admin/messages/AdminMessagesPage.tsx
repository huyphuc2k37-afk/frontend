...
export default function AdminMessagesPage() {
  ...
  const sendReply = async () => {
    ...
    try {
      ...
      if (res.ok) { const data = await res.json(); setMessages((prev) => [...prev, data.message]); setNewMessage(""); }
    } catch {}
    setSending(false);
  };
  ...
  const createConversation = async () => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/messages/conversations`, {
        method: "POST", headers, body: JSON.stringify({ authorId: selectedAuthorId, subject: newSubject.trim(), message: newContent.trim() }),
      });
      if (res.ok) {
        const data = await res.json();
        setShowNew(false); setNewSubject(""); setNewContent(""); setSelectedAuthorId(""); setSearchAuthor("");
        const convRes = await fetch(`${API_BASE_URL}/api/messages/conversations`, { headers });
        if (convRes.ok) { const convData = await convRes.json(); setConversations(convData?.conversations || []); }
    ...
    <div className="flex h-[calc(100vh-120px)] overflow-hidden rounded-2xl bg-white shadow-sm">
      ...
      <div className={`w-full border-r border-gray-200 md:w-80 ${selectedConv ? "hidden md:block" : ""}`}>
        <div className="flex items-center justify-between border-b border-gray-100 p-4">
          <h2 className="text-heading-sm font-bold text-gray-900">Nhắn tin tác giả</h2>
          <button onClick={() => setShowNew(true)} className="rounded-lg bg-indigo-500 p-2 text-white hover:bg-indigo-600"><PlusIcon className="h-4 w-4" /></button>
        </div>
        <div className="overflow-y-auto" style={{ maxHeight: "calc(100vh - 200px)" }}>
          {conversations.length === 0 ? (
            ...
            <button
              ...
              className={`w-full border-b border-gray-50 p-4 text-left transition-colors hover:bg-gray-50 ${selectedConv === conv.id ? "bg-indigo-50" : ""} ${conv.hasUnread ? "bg-blue-50/50" : ""}`}
            >
              <div className="flex items-start gap-3">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between">
                    <p className="truncate text-body-sm font-semibold text-gray-900">{conv.otherParticipants.map((p) => p.name).join(", ")}</p>
                    <span className="text-[10px] text-gray-400">{formatTime(conv.updatedAt)}</span>
                  </div>
                  {conv.subject && <p className="truncate text-caption font-medium text-indigo-600">{conv.subject}</p>}
                  {conv.lastMessage && <p className="mt-0.5 truncate text-caption text-gray-500">{conv.lastMessage.sender.name}: {conv.lastMessage.content}</p>}
                  {conv.hasUnread && <span className="mt-1 inline-block h-2 w-2 rounded-full bg-indigo-500" />}
                </div>
              </div>
            </button>
          ))}
          ...
          <>
            <div className="flex items-center gap-3 border-b border-gray-100 p-4">
              ...
              <div className="flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-indigo-400 to-purple-500">
                <span className="text-body-sm font-bold text-white">{convParticipants.find((p) => p.role === "author")?.name?.charAt(0) || "?"}</span>
              </div>
              <div>
                <p className="text-body-sm font-semibold text-gray-900">{convParticipants.filter((p) => p.role === "author").map((p) => p.name).join(", ") || "Tác giả"}</p>
              ...
              {messages.map((msg) => {
                ...
                return (
                  <div key={msg.id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                    <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 ${isMe ? "bg-indigo-500 text-white" : "bg-gray-100 text-gray-900"}`}>
                      ...
                      <p className={`mt-1 text-[10px] ${isMe ? "text-indigo-200" : "text-gray-400"}`}>{formatTime(msg.createdAt)}</p>
                    </div>
                  </div>
                );
              })}
            ...
            <div className="border-t border-gray-100 p-4">
              <div className="flex items-end gap-2">
                <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); sendReply(); } }} placeholder="Nhập tin nhắn..." rows={1} className="flex-1 resize-none rounded-xl border border-gray-200 px-4 py-2.5 text-body-sm focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-100" />
                <button onClick={sendReply} disabled={!newMessage.trim() || sending} className="rounded-xl bg-indigo-500 p-2.5 text-white hover:bg-indigo-600 disabled:opacity-50"><PaperAirplaneIcon className="h-5 w-5" /></button>
              </div>
            </div>
          </>
        ) : (
          ...
          <div className="mx-4 w-full max-w-md rounded-2xl bg-white p-6 shadow-xl">
            ...
            <div className="mt-4">
              ...
              <div className="relative mt-1">
                ...
                <input value={searchAuthor} onChange={(e) => setSearchAuthor(e.target.value)} placeholder="Nhập tên hoặc email..." className="w-full rounded-lg border border-gray-200 py-2 pl-9 pr-3 text-body-sm focus:border-indigo-400 focus:outline-none" />
              </div>
              {selectedAuthorId && (
                <div className="mt-2 flex items-center gap-2 rounded-lg bg-indigo-50 px-3 py-2">
                  <div className="flex h-7 w-7 items-center justify-center rounded-full bg-indigo-500 text-[11px] font-bold text-white">{searchAuthor.charAt(0)}</div>
              ...
              {authorResults.length > 0 && !selectedAuthorId && (
                <div className="mt-1 max-h-40 overflow-y-auto rounded-lg border border-gray-200 bg-white">
                  {authorResults.map((a) => (
                    <button key={a.id} onClick={() => { setSelectedAuthorId(a.id); setSearchAuthor(a.name); setAuthorResults([]); }} className="w-full px-3 py-2 text-left hover:bg-gray-50">
              ...
              {!selectedAuthorId && searchAuthor.length < 2 && allAuthors.length > 0 && (
                <div className="mt-2">
                  ...
                  <div className="max-h-40 overflow-y-auto rounded-lg border border-gray-100 bg-gray-50">
                    {allAuthors.map((a) => (
                      <button key={a.id} onClick={() => { setSelectedAuthorId(a.id); setSearchAuthor(a.name); }} className="flex w-full items-center gap-2.5 px-3 py-2 text-left transition-colors hover:bg-indigo-50">
                        <div className="flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 text-[11px] font-bold text-white">{a.name.charAt(0)}</div>
            ...
            <div className="mt-3">
              ...
              <input value={newSubject} onChange={(e) => setNewSubject(e.target.value)} placeholder="VD: Về truyện ABC..." className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-body-sm focus:border-indigo-400 focus:outline-none" />
            </div>
            <div className="mt-3">
              <label className="text-body-sm font-medium text-gray-700">Nội dung tin nhắn</label>
              <textarea value={newContent} onChange={(e) => setNewContent(e.target.value)} placeholder="Nhập nội dung..." rows={4} className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-body-sm focus:border-indigo-400 focus:outline-none" />
            </div>
            <div className="mt-5 flex justify-end gap-2">
              <button onClick={() => { setShowNew(false); setSearchAuthor(""); setSelectedAuthorId(""); setNewSubject(""); setNewContent(""); }} className="rounded-lg border border-gray-200 px-4 py-2 text-body-sm text-gray-600 hover:bg-gray-50">Hủy</button>
              <button onClick={createConversation} disabled={!selectedAuthorId || !newContent.trim() || sending} className="rounded-lg bg-indigo-500 px-4 py-2 text-body-sm font-medium text-white hover:bg-indigo-600 disabled:opacity-50">{sending ? "Đang gửi..." : "Gửi tin nhắn"}</button>
            </div>
          </div>
        </div>
      )}

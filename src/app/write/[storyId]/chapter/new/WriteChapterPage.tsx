...
export default function WriteChapterPage() {
  ...
  const handleSubmit = async () => {
    ...
    setError("");
    try {
      const res = await fetch(`${API_BASE_URL}/api/manage/stories/${storyId}/chapters`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        ...
        setError(data.error || "Không thể tạo chương");
        setSaving(false);
        return;
      }
    ...
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-heading-md font-bold text-gray-900">Viết chương mới</h2>
            <p className="text-caption text-gray-500">{wordCount} chữ</p>
          </div>
        </div>
        <button
